...
$rezultate_maxime = 10;
$intrari_totale = mysqli_num_rows(mysqli_query($conexiune, 'SELECT `id` FROM `produse` WHERE `cat`="' . $cat . '"'));
...
if ($intrari_totale == 0) {
	...
	$cerereSQL = 'SELECT * FROM `produse` WHERE `cat`="' . $cat . '" ORDER BY `id` DESC LIMIT ' . $from . ', ' . $rezultate_maxime . '';
